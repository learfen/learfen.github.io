...
window.cell = gameScreen().width / 100;
...
class Movimiento {
	...
	static activar(objeto, nombreMovimiento) {
		objeto.etiqueta.style.left = "0px";
		objeto.etiqueta.style.top = "0px";
		objeto.etiqueta.classList.add("animar");
		if (typeof movimientos[nombreMovimiento]["animar"] == "function")
			return movimientos[nombreMovimiento].animar(
				objeto.etiqueta,
				objeto
			);
		objeto.etiqueta.style.transform =
			movimientos[nombreMovimiento].animation;
	}
	static aplicarGravedad(etiqueta, acciones, objeto) {
		let interval = setInterval(() => {
			...
			let [x, y] = etiqueta.style.transform
				.split("translate(")[1]
				.split(")")[0]
				.split(",");
			y = +y.replace("px", "");
			if (y < gameScreen().height) {
				...
				if (acciones) {
					if (acciones["colision"])
						acciones["colision"](objeto, Juego.colisiones(objeto));
				}
				return 0;
			}
			if (acciones) {
	...
	static crear(nombreMovimiento, efecto, animar) {
		...
		if (efecto["x"] != undefined && efecto["y"] != undefined) {
			animation += ` translate(${efecto.x * cell + 1}px, ${
				efecto.y * cell + 1
			}px) `;
...
Movimiento.crear("centrar", { x: 50, y: 50 }, (etiqueta) => {
	etiqueta.style.transform = `translate(${
		movimientos.centrar.description.x - etiqueta.offsetWidth / 2
	}px, ${movimientos.centrar.description.y - etiqueta.offsetHeight / 2}px)`;
});

function escucharTeclas( keys , next ){
	let translateKey = {
		'arriba':'ArrowUp',
		'abajo':'ArrowDown',
		'izquierda':'ArrowLeft',
	...
	for(let key of keys){
		if(translateKey[key] != undefined) {
			original[translateKey[key]] = key
			useKeys.push(translateKey[key]) 
		}else {
			useKeys.push(key)
		}
	}
	document.addEventListener('keyup' , event => {
		if(useKeys.includes(event.key)) {
			next(original[event.key] ? original[event.key] : event.key)
		}
	})
}
...
class Componente extends Movimiento {
	...
	imagen(url) {
		...
		this.etiqueta.style.background = this.etiqueta.style.background.replace(
			img,
			url
		);
	}
	...
	escucharTeclas(keys , next ){
		escucharTeclas(keys , key => next( this , key ) )
	}
	colorPixel(){}
	escucharColor() {
		...
		let img = new Image();
		let self = this
		img.src = this.etiqueta.style.background.split("url(")[1].split(")")[0].slice(3 , -1)
		img.onload = () => {
			let tag = document.querySelector('#'+this.id)
			canvas.width = tag.style.width.replace("px", "");
			canvas.height = tag.style.height.replace("px", "");
			context.drawImage(img, 0, 0);
			self.colorPixel = ( x, y ) => {
				try {
					x = parseInt( window.cell * x * 1.1);
					y = parseInt( window.cell * y * 1.1);
	...
	usarMovimiento(name, action) {
		if (movimientos[name] != undefined)
			return Movimiento.activar(this, name);
		Movimiento[name](this.etiqueta, action, this);
		return this;
	}
	trayectoria(efecto, time) {
		const self = this;
		this.etiqueta.classList.add("animar-trayectoria");

		self.etiqueta.style.transition = "all .25s ease";
		function insertarIntermedios(array) {
			let nuevoArray = [];
			for (let i = 0; i < array.length; i++) {
				...
				if (i < array.length - 1) {
					...
					let diferenciaX = Math.abs(actual.x - siguiente.x);
					let diferenciaY = Math.abs(actual.y - siguiente.y);
					if (diferenciaX > 2 || diferenciaY > 2) {
						let intermedio = {
							x: (actual.x + siguiente.x) / 2,
							y: (actual.y + siguiente.y) / 2,
						};
						nuevoArray.push(intermedio);
					}
				}
			}
			if (array.length != nuevoArray.length)
				return insertarIntermedios(nuevoArray);
			return nuevoArray;
		}
		efecto = insertarIntermedios(efecto);
		const start = (i) => {
			if (i == efecto.length) return 0;
			self.moverA(efecto[i]);
			setTimeout(() => start(++i), 1000 * time);
		};
		setTimeout(() => start(0), 200 * time);
		return this
	}
	moverA({ x, y }) {
		...
		this.etiqueta.style.transform =
			start[0] +
			end +
			`translate(${parseInt(x * cell) + 1}px, ${
				parseInt(y * cell) + 1
			}px)`;
		this.x = x
		this.y = y
	...
	personalizado(custom) {
		if (custom["clases"])
			this.etiqueta.className += " " + custom.clases + " ";
		if (custom["estilos"])
			this.etiqueta.style += ";" + custom.estilos + ";";
		return this;
	}
	colorearTexto(color) {
...
class Info {
	constructor(type, title, value, actualizacion) {
		...
		let classType = {
			...
			barra() {
				...
				bar.style =
					"min-height:10px;transition: .5s all;width:" + value + "%";
				container.style =
					"margin-top:-9px;border:1px #aaa solid;width:100px;min-height:10px";
	...
	incrementar(cantidad) {
		...
		if (this.type == "barra") {
			let points =
				+this.etiquetaValue.style.width.replace("%", "") + +cantidad;
...
class Juego {
	...
	static insertar(data) {
		...
		ima.src = data.url;
		ima.style =
			"z-index:-1;width:100px;height:auto;opacity:0;position:absolute";
		ima.onload = (e) => {
		...
		if (data["customClass"]) imagen.className += data["customClass"];
		if (data["option"]) imagen.classList.add("item-option");
		...
		if (data["arrastrable"]) {
			...
			imagen.addEventListener("dragstart", (ev) => {
...
class Mochila {
	constructor() {
		...
		mochilaImage.style =
			'background-image: url("./images/mochila.png");background-size: contain;background-clip: padding-box;width:80px;min-height:80px;max-width:80px';
		mochilaImage.className =
			"position-fixed bottom-0 start-0 btn ms-2 mb-3";
	...
	guardar(etiqueta) {
		...
		setTimeout(() => {
			...
			tooltop.innerHTML =
				juego[etiqueta.getAttribute("name")].descripcion;
			...
			buttonUsar.onclick = () => {
				if (juego[etiqueta.getAttribute("name")].usar)
					juego[etiqueta.getAttribute("name")].usar(
						juego[etiqueta.getAttribute("name")]
					);
	...
	abrir() {
		...
		const renderItem = (index) => {
			...
			let rel =
				+this.items[index].style.height.replace("px", "") /
				+this.items[index].style.width.replace("px", "");
			this.items[index].style.height = 100 * rel + "px";
			this.items[index].style.width = "100px";
			this.items[index].style.transform = `translate(${
				+(100 * count) + 10
			}px,${+(140 * row) + marginTop}px)`;
...
function main() {
	...
	if (mode == "creando") {
		document.addEventListener("mousemove", (event) => {
			coordenadas.innerHTML = `{"x":${parseInt(
				event.clientX / cell
			)},"y": ${parseInt(event.clientY / cell)}, "pixel":{"x":${
				event.clientX
			},"y":${event.clientY}}}`;
		});
